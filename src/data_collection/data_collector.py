...
def get_pr_comments(repo, pr_number):
    """Fetches all issue comments associated with a pull request (excluding bots)."""
...
def get_pr_reviews(repo, pr_number):
    """Fetches all reviews for a pull request (excluding bots)."""
...
def collect_repo_data(repo):
    ...
    pr_data, comment_data, review_data = [], [], []
    for issue in pr_issues:
        ...
        if pr:
            ...
            # Process Reviews: Only include those up to the PR's closure time
